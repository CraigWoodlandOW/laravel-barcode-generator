...
namespace CraigWoodlandOW\LaravelBarcodeGenerator\Barcodes;

use CraigWoodlandOW\LaravelBarcodeGenerator\Utilities\Converters;
use CraigWoodlandOW\LaravelBarcodeGenerator\Utilities\Validators;

class ean13
{
    function create($upc, $imgtype = "png", $outputimage = true, $resize = 1, $resizetype = "resize", $outfile = NULL, $hidecd = false)
    {
        if(!isset($upc)) {
            return false;
        }
        ...
        if(strlen($upc) == 8) {
            $upc = Converters::convert_upce_to_ean13($upc);
        }

        if(strlen($upc) == 12) {
            $upc = Converters::convert_upca_to_ean13($upc);
        }

        if(strlen($upc) == 12 && Validators::validate_upca($upc) === true) {
            $upc = "0" . $upc;
        }

        if(strlen($upc) == 12 && Validators::validate_upca($upc) === false) {
            $upc = $upc . Validators::validate_ean13($upc, true);
        }
        ...
        if(Validators::validate_ean13($upc) === false) {
            preg_match("/^(\d{12})/", $upc, $pre_matches);
            $upc = $pre_matches[1] . Validators::validate_ean13($pre_matches[1], true);
        }
